...
_DEFAULT_WALLPAPER_OPACITY = 0.85
_DEFAULT_WALLPAPER_BLUR = 0
_DEFAULT_WALLPAPER_BRIGHTNESS = 1.1
...
def create_app() -> FastAPI:
    ...
    def _avatar_dir() -> Path:
        """Return the directory for uploaded persona avatars."""
        return get_config_dir() / "personas" / "avatars"

    def _default_avatar_dir() -> Path:
        """Return the directory for built-in default persona avatars."""
        return get_config_dir() / "personas" / "default_avatars"

    def _find_default_avatar_source(persona_id: str) -> Path | None:
        """Find a default avatar image for a persona.

        Priority:
        1. Built-in default avatar (e.g. default_avatars/exusiai.png).
        2. Live2D texture_00.png.
        3. Any available fallback.
        """
        default_dir = _default_avatar_dir()
        default_candidate = default_dir / f"{persona_id}.png"
        if default_candidate.exists():
            return default_candidate

        project_root = Path(__file__).resolve().parents[2]
        persona_textures = {
            "exusiai": project_root / "exusiai_live2d/00.4096/texture_00.png",
            "kal'tsit": project_root / "kal'tsit_live2d/凯尔希直播版1.4096/texture_00.png",
        }
        candidate = persona_textures.get(persona_id)
        if candidate is not None and candidate.exists():
            return candidate
        # Fallback to any available texture.
        for c in persona_textures.values():
            if c.exists():
                return c
        return None

    def _media_type_for_avatar_ext(ext: str) -> str:
        """Return the HTTP media type for an avatar image extension."""
        ext = ext.lower()
        if ext in ("jpg", "jpeg"):
            return "image/jpeg"
        if ext == "webp":
    ...
    def _ensure_default_avatar() -> Path:
        """Return a generated default avatar PNG, creating it if necessary."""
        avatar_dir = _avatar_dir()
        avatar_dir.mkdir(parents=True, exist_ok=True)
        default_path = avatar_dir / "_default.png"
        ...
        try:
            ...
            img.save(default_path, format="PNG")
        except Exception:
            # Fallback to a 1x1 transparent PNG if Pillow is unavailable.
    ...
    async def get_personas() -> JSONResponse:
        ...
        for persona in personas:
            persona["avatar_url"] = f"/api/personas/{persona.get('id')}/avatar"
    ...
    async def get_persona(persona_id: str) -> JSONResponse:
        ...
        return JSONResponse(
            content={
                ...
                "persona": persona,
                "avatar_url": f"/api/personas/{persona_id}/avatar",
            }
        )
    ...
    async def upload_persona_avatar(
        ...
        """Upload a custom avatar image for a persona."""
        if not persona_exists(persona_id):
            return JSONResponse(status_code=404, content={"error": "persona_not_found"})
        if not file.filename:
            return JSONResponse(status_code=400, content={"error": "missing_filename"})
        ext = Path(file.filename).suffix.lower().lstrip(".")
        if ext not in {"jpg", "jpeg", "png", "webp"}:
            return JSONResponse(status_code=400, content={"error": "invalid_extension"})
        content = await file.read()
        if len(content) > 2 * 1024 * 1024:
            return JSONResponse(status_code=400, content={"error": "file_too_large"})
        avatar_dir = _avatar_dir()
        avatar_dir.mkdir(parents=True, exist_ok=True)
        for existing in avatar_dir.glob(f"{persona_id}.*"):
            existing.unlink()
        target = avatar_dir / f"{persona_id}.{ext}"
        target.write_bytes(content)
        timestamp = int(target.stat().st_mtime)
        return JSONResponse(
            content={"ok": True, "url": f"/api/personas/{persona_id}/avatar?t={timestamp}"}
        )

    @app.get("/api/personas/{persona_id}/avatar")
    async def get_persona_avatar(persona_id: str) -> FileResponse:
        """Serve a persona's uploaded avatar, Live2D texture, or generated default."""
        avatar_dir = _avatar_dir()
        for ext in ("png", "jpg", "jpeg", "webp"):
            avatar_path = avatar_dir / f"{persona_id}.{ext}"
            if avatar_path.exists():
                return FileResponse(
                    avatar_path, media_type=_media_type_for_avatar_ext(ext)
                )
        fallback = _find_default_avatar_source(persona_id)
        if fallback is not None and fallback.exists():
            return FileResponse(fallback, media_type="image/png")
        return FileResponse(_ensure_default_avatar(), media_type="image/png")

    @app.delete("/api/personas/{persona_id}/avatar")
    async def delete_persona_avatar(persona_id: str) -> JSONResponse:
        """Remove the uploaded avatar for a persona."""
        avatar_dir = _avatar_dir()
        for ext in ("png", "jpg", "jpeg", "webp"):
            avatar_path = avatar_dir / f"{persona_id}.{ext}"
            if avatar_path.exists():
